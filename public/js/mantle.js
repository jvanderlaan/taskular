!function(t) {
    ...
    var h = t(window), d = window.document, c = t(d), u = window.Number, g = "datepicker", p = "click.datepicker", f = /(y|m|d)+/g, m = /\d+/g, v = /^\d{2,4}$/, w = [ "datepicker-top-left", "datepicker-top-right", "datepicker-bottom-left", "datepicker-bottom-right" ].join(" "), y = "datepicker-hide", $ = 0, k = 1, D = 2, C = Math.min, b = Object.prototype.toString;
    l.prototype = {
        constructor: l,
        init: function() {
            ...
            this.isInline = e.inline && (e.container || !this.isInput), this.format = function(t) {
                var e, i, a = String(t).toLowerCase(), s = a.match(f);
        ...
        bind: function() {
            ...
            this.isInline || (e.trigger ? this.$trigger.on(p, t.proxy(this.toggle, this)) : this.isInput ? i.on("focus.datepicker", t.proxy(this.show, this)) : i.on(p, t.proxy(this.show, this)));
        },
        unbind: function() {
            var e = this.options, i = this.$element;
            ...
            this.isInline || (e.trigger ? this.$trigger.off(p, this.toggle) : this.isInput ? i.off("focus.datepicker", this.show) : i.off(p, this.show));
        },
        showView: function(t) {
            ...
            if (s.hasYear || s.hasMonth || s.hasDay) switch (u(t)) {
              case D:
              case "years":
                i.addClass(y), a.addClass(y), s.hasYear ? (this.fillYears(), e.removeClass(y), this.place()) : this.showView($);
                break;

              case k:
              case "months":
                e.addClass(y), a.addClass(y), s.hasMonth ? (this.fillMonths(), i.removeClass(y), 
                this.place()) : this.showView(D);
                break;

              default:
                e.addClass(y), i.addClass(y), s.hasDay ? (this.fillDays(), a.removeClass(y), this.place()) : this.showView(k);
            }
        },
        hideView: function() {
        ...
        place: function() {
            if (!this.isInline) {
                ...
                d + r > a && (d = d + n - r, p = p.replace("left", "right")), i.removeClass(w).addClass(p).css({
                    top: u,
                    left: d,
        ...
        click: function(e) {
            ...
            a = d.getMonth(), s = d.getDate(), r = l.data("view")) {
              case "years prev":
              case "years next":
                i = "years prev" === r ? i - 10 : i + 10, o = l.text(), (n = v.test(o)) && (i = parseInt(o, 10), 
                this.date = new Date(i, a, C(s, 28))), this.viewDate = new Date(i, a, C(s, 28)), 
                this.fillYears(), n && (this.showView(k), this.pick("year"));
                break;

              case "year prev":
              case "year next":
                i = "year prev" === r ? i - 1 : i + 1, this.viewDate = new Date(i, a, C(s, 28)), 
                this.fillMonths();
                break;
              ...
              case "year picked":
                this.format.hasMonth ? this.showView(k) : (l.addClass(h.pickedClass).siblings().removeClass(h.pickedClass), 
                this.hideView()), this.pick("year");
                break;

              case "year":
                i = parseInt(l.text(), 10), this.date = new Date(i, a, C(s, 28)), this.format.hasMonth ? (this.viewDate = new Date(i, a, C(s, 28)), 
                this.showView(k)) : (l.addClass(h.pickedClass).siblings().removeClass(h.pickedClass), 
                this.hideView()), this.pick("year");
                break;

              case "month prev":
              case "month next":
                a = "month prev" === r ? a - 1 : "month next" === r ? a + 1 : a, this.viewDate = new Date(i, a, C(s, 28)), 
                this.fillDays();
                break;

              case "month current":
                this.format.hasMonth && this.showView(k);
                break;

              case "month picked":
                this.format.hasDay ? this.showView($) : (l.addClass(h.pickedClass).siblings().removeClass(h.pickedClass), 
                this.hideView()), this.pick("month");
                break;

              case "month":
                a = t.inArray(l.text(), h.monthsShort), this.date = new Date(i, a, C(s, 28)), this.format.hasDay ? (this.viewDate = new Date(i, a, C(s, 28)), 
        ...
        show: function() {
            ...
            this.$picker.removeClass(y).on(p, t.proxy(this.click, this)), this.showView(this.options.startView), 
            this.isInline || (h.on("resize.datepicker", this._place = o(this.place, this)), 
            c.on(p, this._clickDoc = o(this.clickDoc, this)), c.on("keyup.datepicker", this._keyupDoc = o(this.keyupDoc, this)), 
            this.place()));
        },
        hide: function() {
            this.isShown && (this.trigger("hide.datepicker").isDefaultPrevented() || (this.isShown = !1, 
            this.$picker.addClass(y).off(p, this.click), this.isInline || (h.off("resize.datepicker", this._place), 
            c.off(p, this._clickDoc), c.off("keyup.datepicker", this._keyupDoc))));
        },
        toggle: function() {
        ...
        parseDate: function(t) {
            ...
            if (e(t) && (d = t.match(m) || []), t = new Date(), a = t.getFullYear(), n = t.getDate(), 
        ...
        destroy: function() {
            this.unbind(), this.unbuild(), this.$element.removeData(g);
        }
        ...
        return this.each(function() {
            var a, n, r = t(this), h = r.data(g);
            if (!h) {
                if (/destroy/.test(i)) return;
                a = t.extend({}, r.data(), t.isPlainObject(i) && i), r.data(g, h = new l(this, a));
            }
...
var dropdowns = function() {
    ...
    "use strict";
    function t(t) {
        s.$modalSelected.toggleClass("toggled"), s.$body.height() > $(window).height() && (s.$body.toggleClass("modal-active"), 
    ...
    return {
        init: function() {
            s.$modal = $(".modal"), s.$modalSelected = $("[id^=modal]"), s.$modalToggle = $(".modal-toggle"), 
